...
- 어느 기존 카테고리에도 의미상 맞지 않으면 새 카테고리를 제안한다(is_new=true).
- 억지로 끼워넣지 마라. 기존 카테고리와 '약하게만' 맞을 때(확신이 낮을 때)는
  기존에 우겨넣지 말고 새 카테고리를 제안하는 쪽을 택한다.
- 새 카테고리 이름은 메모와 같은 언어로, 짧고 일반적인 명사로.
- 확신 정도를 confidence(0~1)로 정직하게 준다.`;
